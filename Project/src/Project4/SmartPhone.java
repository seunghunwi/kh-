...
public class SmartPhone {
	...
	public Contact   inputContactData1() {  // 키보드로 입력 받아 객체를 생성한다.
		...
		System.out.print("직급 :");
		String  rank = sc.next();
		return new CumstomContact(name, phoneNumber, email, account, birth, group, businessname, transactionItems, rank);	
	...
	public Contact   inputContactData() {  // 키보드로 입력 받아 객체를 생성한다.   
		Scanner sc = new Scanner(System.in);
	...
	public void addContact(Contact contact){// 배열에 연락처 객체 저장
	...
	public void printAllConatact() { // 모든 연락처 출력
		for(int i=0; i<num; i++) {
			ct[i].printinfo();
		}		
	}
	...
	public void editContact(String name) { // 연락처 수정
		for(int i=0; i<num; i++) {
			if(ct[i].getPhonename().contentEquals(name)) {
